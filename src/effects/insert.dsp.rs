...
impl insertSIG0 {
	...
	fn instance_initinsertSIG0(&mut self, sample_rate: i32) {
		let mut l20: i32 = 0;
		loop {
			self.iVec4[l20 as usize] = 0;
			l20 = (l20 + 1);
			if (l20 < 2) { continue; } else { break; }
		}
		let mut l21: i32 = 0;
		loop {
			self.iRec21[l21 as usize] = 0;
			l21 = (l21 + 1);
...
impl FaustDsp for insert {
	...
	fn instance_clear(&mut self) {
		...
		for l38 in 0..2 {
			self.fVec7[l38 as usize] = 0.0;
		}
		for l39 in 0..4096 {
			self.fVec8[l39 as usize] = 0.0;
		}
		for l40 in 0..2 {
		...
		for l50 in 0..2 {
			self.fVec9[l50 as usize] = 0.0;
		}
		for l51 in 0..4096 {
			self.fVec10[l51 as usize] = 0.0;
		}
		for l52 in 0..2 {
		...
		for l58 in 0..3 {
			self.fRec5[l58 as usize] = 0.0;
		}
		for l59 in 0..2 {
			self.fRec54[l59 as usize] = 0.0;
		}
		for l60 in 0..3 {
		...
		for l64 in 0..3 {
			self.fRec4[l64 as usize] = 0.0;
		}
		for l65 in 0..2 {
			self.fVec12[l65 as usize] = 0.0;
		}
		for l66 in 0..2 {
			self.fRec58[l66 as usize] = 0.0;
		}
		for l67 in 0..2 {
	...
	fn get_param(&self, param: ParamIndex) -> Option<Self::T> {
		match param.0 {
			...
			25 => Some(self.fEntry10),
			23 => Some(self.fEntry11),
			27 => Some(self.fEntry12),
			28 => Some(self.fEntry13),
			26 => Some(self.fEntry14),
			30 => Some(self.fEntry15),
			31 => Some(self.fEntry16),
			29 => Some(self.fEntry17),
			10 => Some(self.fEntry18),
			11 => Some(self.fEntry19),
			20 => Some(self.fEntry2),
			13 => Some(self.fEntry20),
			14 => Some(self.fEntry21),
			7 => Some(self.fEntry22),
			1 => Some(self.fEntry23),
			8 => Some(self.fEntry24),
			16 => Some(self.fEntry25),
			12 => Some(self.fEntry26),
			3 => Some(self.fEntry27),
			5 => Some(self.fEntry28),
			18 => Some(self.fEntry29),
			21 => Some(self.fEntry3),
			19 => Some(self.fEntry4),
			9 => Some(self.fEntry5),
			0 => Some(self.fEntry6),
			22 => Some(self.fEntry7),
			2 => Some(self.fEntry8),
			24 => Some(self.fEntry9),
			_ => None,
		}
	}
	...
	fn set_param(&mut self, param: ParamIndex, value: Self::T) {
		match param.0 {
			...
			25 => { self.fEntry10 = value }
			23 => { self.fEntry11 = value }
			27 => { self.fEntry12 = value }
			28 => { self.fEntry13 = value }
			26 => { self.fEntry14 = value }
			30 => { self.fEntry15 = value }
			31 => { self.fEntry16 = value }
			29 => { self.fEntry17 = value }
			10 => { self.fEntry18 = value }
			11 => { self.fEntry19 = value }
			20 => { self.fEntry2 = value }
			13 => { self.fEntry20 = value }
			14 => { self.fEntry21 = value }
			7 => { self.fEntry22 = value }
			1 => { self.fEntry23 = value }
			8 => { self.fEntry24 = value }
			16 => { self.fEntry25 = value }
			12 => { self.fEntry26 = value }
			3 => { self.fEntry27 = value }
			5 => { self.fEntry28 = value }
			18 => { self.fEntry29 = value }
			21 => { self.fEntry3 = value }
			19 => { self.fEntry4 = value }
			9 => { self.fEntry5 = value }
			0 => { self.fEntry6 = value }
			22 => { self.fEntry7 = value }
			2 => { self.fEntry8 = value }
			24 => { self.fEntry9 = value }
			_ => {}
		}
	}
	...
	fn compute(&mut self, count: i32, inputs: &[&[Self::T]], outputs: &mut[&mut[Self::T]]) {
		...
		let mut fSlow6: F32 = (((((fSlow5 == ((1 | (fSlow5 as i32)) as F32)) as i32) == 2) as i32) as F32);
		let mut fSlow7: F32 = (self.fButton1 as F32);
		let mut fSlow8: F32 = (self.fEntry4 as F32);
		let mut fSlow9: F32 = (0.0833333358 * fSlow6);
		let mut iSlow10: i32 = ((self.fEntry5 as F32) as i32);
		let mut fSlow11: F32 = (self.fEntry6 as F32);
		let mut fSlow12: F32 = (self.fEntry7 as F32);
		let mut fSlow13: F32 = (self.fEntry8 as F32);
		let mut iSlow14: i32 = ((fSlow0 == 0.0) as i32);
		let mut fSlow15: F32 = (self.fEntry9 as F32);
		let mut fSlow16: F32 = (self.fEntry10 as F32);
		let mut fSlow17: F32 = (((fSlow16 >= 3.0) as i32) as F32);
		let mut fSlow18: F32 = (((fSlow16 >= 2.0) as i32) as F32);
		let mut fSlow19: F32 = (((fSlow16 >= 1.0) as i32) as F32);
		let mut fSlow20: F32 = (self.fEntry11 as F32);
		let mut fSlow21: F32 = (((fSlow16 >= 4.0) as i32) as F32);
		...
		for (((((((((((input0, input1), input2), input3), output0), output1), output2), output3), output4), output5), output6), output7) in zipped_iterators {
			...
			self.fRec7[0] = ((self.fConst1 * (fTemp3 * self.fRec7[1])) + (fSlow4 * fTemp4));
			let mut fTemp5: F32 = (*input2 as F32);
			let mut fTemp6: F32 = (self.fRec8[1] + 1.0);
			self.fRec9[0] = ((self.fConst1 * (fTemp3 * self.fRec9[1])) + (fSlow8 * fTemp4));
			let mut fTemp7: F32 = (fSlow7 + ((0.100000001 * self.fRec9[0]) + -69.0));
			let mut fTemp8: F32 = ((((fSlow6 * fTemp7) == 0.0) as i32) as F32);
			...
			self.fRec10[0] = libm::fmodf((self.fRec10[1] + (1001.0 - F32::powf(2.0, (fSlow9 * fTemp7)))), 1000.0);
			...
			self.fRec11[0] = (fSlow0 + (self.fRec11[1] * (((self.fVec0[1] >= fSlow0) as i32) as F32)));
			self.fRec12[0] = ((self.fConst1 * (fTemp3 * self.fRec12[1])) + (fSlow11 * fTemp4));
			...
			self.iRec15[0] = (iSlow14 * (self.iRec15[1] + 1));
			...
			let mut fTemp30: F32 = if (iSlow10 as i32 != 0) { fTemp29 } else { 1.0 };
			self.fRec17[0] = ((self.fConst1 * (fTemp3 * self.fRec17[1])) + (fSlow15 * fTemp4));
			let mut fTemp31: F32 = (self.fRec18[1] + 1.0);
			let mut fTemp32: F32 = (self.fRec18[1] + -1.0);
			self.fRec18[0] = if (((fTemp31 < fSlow17) as i32) as i32 != 0) { fTemp31 } else { if (((fTemp32 > fSlow17) as i32) as i32 != 0) { fTemp32 } else { fSlow17 } };
			let mut fTemp33: F32 = (self.fRec19[1] + 1.0);
			let mut fTemp34: F32 = (self.fRec19[1] + -1.0);
			self.fRec19[0] = if (((fTemp33 < fSlow18) as i32) as i32 != 0) { fTemp33 } else { if (((fTemp34 > fSlow18) as i32) as i32 != 0) { fTemp34 } else { fSlow18 } };
			let mut fTemp35: F32 = (self.fRec20[1] + 1.0);
			let mut fTemp36: F32 = (self.fRec20[1] + -1.0);
			self.fRec20[0] = if (((fTemp35 < fSlow19) as i32) as i32 != 0) { fTemp35 } else { if (((fTemp36 > fSlow19) as i32) as i32 != 0) { fTemp36 } else { fSlow19 } };
			self.fRec23[0] = ((self.fConst1 * (fTemp3 * self.fRec23[1])) + (fSlow20 * fTemp4));
			let mut fTemp37: F32 = F32::powf(2.0, (0.0833333358 * (fSlow7 + ((0.00999999978 * self.fRec23[0]) + -69.0))));
			let mut fTemp38: F32 = (self.fRec22[1] + (self.fConst3 * fTemp37));
			self.fRec22[0] = (fTemp38 - F32::floor(fTemp38));
			let mut fTemp39: F32 = (self.iVec1[1] as F32);
			...
			self.fRec28[0] = if (((fTemp54 < fSlow21) as i32) as i32 != 0) { fTemp54 } else { if (((fTemp55 > fSlow21) as i32) as i32 != 0) { fTemp55 } else { fSlow21 } };
			self.iRec29[0] = ((1103515245 * self.iRec29[1]) + 12345);
			let mut fTemp56: F32 = (self.iRec29[0] as F32);
			self.fRec30[0] = ((self.fConst1 * (fTemp3 * self.fRec30[1])) + (fSlow22 * fTemp4));
			let mut fTemp57: F32 = (self.fRec31[1] + 1.0);
			let mut fTemp58: F32 = (self.fRec31[1] + -1.0);
			self.fRec31[0] = if (((fTemp57 < fSlow24) as i32) as i32 != 0) { fTemp57 } else { if (((fTemp58 > fSlow24) as i32) as i32 != 0) { fTemp58 } else { fSlow24 } };
			let mut fTemp59: F32 = (self.fRec32[1] + 1.0);
			let mut fTemp60: F32 = (self.fRec32[1] + -1.0);
			self.fRec32[0] = if (((fTemp59 < fSlow25) as i32) as i32 != 0) { fTemp59 } else { if (((fTemp60 > fSlow25) as i32) as i32 != 0) { fTemp60 } else { fSlow25 } };
			let mut fTemp61: F32 = (self.fRec33[1] + 1.0);
			let mut fTemp62: F32 = (self.fRec33[1] + -1.0);
			self.fRec33[0] = if (((fTemp61 < fSlow26) as i32) as i32 != 0) { fTemp61 } else { if (((fTemp62 > fSlow26) as i32) as i32 != 0) { fTemp62 } else { fSlow26 } };
			self.fRec35[0] = ((self.fConst1 * (fTemp3 * self.fRec35[1])) + (fSlow27 * fTemp4));
			let mut fTemp63: F32 = F32::powf(2.0, (0.0833333358 * (fSlow7 + ((0.00999999978 * self.fRec35[0]) + -69.0))));
			...
			self.fRec40[0] = if (((fTemp79 < fSlow28) as i32) as i32 != 0) { fTemp79 } else { if (((fTemp80 > fSlow28) as i32) as i32 != 0) { fTemp80 } else { fSlow28 } };
			self.fRec41[0] = ((self.fConst1 * (fTemp3 * self.fRec41[1])) + (fSlow29 * fTemp4));
			let mut fTemp81: F32 = (self.fRec42[1] + 1.0);
			let mut fTemp82: F32 = (self.fRec42[1] + -1.0);
			self.fRec42[0] = if (((fTemp81 < fSlow31) as i32) as i32 != 0) { fTemp81 } else { if (((fTemp82 > fSlow31) as i32) as i32 != 0) { fTemp82 } else { fSlow31 } };
			let mut fTemp83: F32 = (self.fRec43[1] + 1.0);
			let mut fTemp84: F32 = (self.fRec43[1] + -1.0);
			self.fRec43[0] = if (((fTemp83 < fSlow32) as i32) as i32 != 0) { fTemp83 } else { if (((fTemp84 > fSlow32) as i32) as i32 != 0) { fTemp84 } else { fSlow32 } };
			let mut fTemp85: F32 = (self.fRec44[1] + 1.0);
			let mut fTemp86: F32 = (self.fRec44[1] + -1.0);
			self.fRec44[0] = if (((fTemp85 < fSlow33) as i32) as i32 != 0) { fTemp85 } else { if (((fTemp86 > fSlow33) as i32) as i32 != 0) { fTemp86 } else { fSlow33 } };
			self.fRec46[0] = ((self.fConst1 * (fTemp3 * self.fRec46[1])) + (fSlow34 * fTemp4));
			let mut fTemp87: F32 = F32::powf(2.0, (0.0833333358 * (fSlow7 + ((0.00999999978 * self.fRec46[0]) + -69.0))));
			...
			self.fRec6[0] = ((self.fVec11[1] * fTemp111) + (((fTemp107 / fTemp108) - (self.fRec6[1] * fTemp112)) / fTemp110));
			...
			self.fRec5[0] = (self.fRec6[0] - (((self.fRec5[2] * fTemp113) + (2.0 * (self.fRec5[1] * fTemp115))) / fTemp116));
			let mut fTemp117: F32 = (0.0 - (2.0 / fTemp114));
			self.fRec54[0] = (0.0 - (((fTemp112 * self.fRec54[1]) - (fTemp107 + self.fVec11[1])) / fTemp110));
			self.fRec53[0] = (self.fRec54[0] - (((fTemp113 * self.fRec53[2]) + (2.0 * (fTemp115 * self.fRec53[1]))) / fTemp116));
			self.fRec55[0] = ((self.fConst1 * (fTemp3 * self.fRec55[1])) + (fSlow37 * fTemp4));
			let mut fTemp118: F32 = F32::powf(10.0, F32::log10(F32::powf(8.0, (0.0399999991 * self.fRec55[0]))));
			self.fRec56[0] = ((self.fConst1 * (fTemp3 * self.fRec56[1])) + (fSlow38 * fTemp4));
			let mut fTemp119: F32 = F32::powf(2.0, (0.0833333358 * self.fRec56[0]));
			...
			let mut fTemp122: F32 = (20.0 * F32::log10(F32::powf(8.0, (0.0399999991 * self.fRec57[0]))));
			...
			let mut fTemp130: F32 = (2.0 * (self.fRec4[1] * fTemp129));
			let mut fTemp131: F32 = (((fTemp121 + fTemp127) / fTemp120) + 1.0);
			self.fRec4[0] = ((((((self.fRec5[1] * fTemp117) + (self.fRec5[0] / fTemp114)) + (self.fRec5[2] / fTemp114)) + ((self.fRec53[2] + (self.fRec53[0] + (2.0 * self.fRec53[1]))) * fTemp118)) / fTemp116) - (((self.fRec4[2] * fTemp128) + fTemp130) / fTemp131));
			let mut fTemp132: F32 = if (iTemp123 as i32 != 0) { fTemp125 } else { fTemp126 };
			let mut fTemp133: F32 = (((fTemp121 + fTemp132) / fTemp120) + 1.0);
			let mut fTemp134: F32 = (1.0 - ((fTemp132 - fTemp121) / fTemp120));
			let mut fTemp135: F32 = ((fTemp130 + (self.fRec4[0] * fTemp133)) + (self.fRec4[2] * fTemp134));
			let mut fTemp136: F32 = (fTemp135 / fTemp131);
			self.fVec12[0] = fTemp136;
			self.fRec58[0] = ((self.fConst1 * (self.fRec58[1] * fTemp3)) + (fSlow40 * fTemp4));
			self.fRec59[0] = ((self.fConst1 * (fTemp3 * self.fRec59[1])) + (fSlow41 * fTemp4));
			let mut fTemp137: F32 = F32::tan((self.fConst12 * F32::powf(2.0, (0.0833333358 * (self.fRec58[0] - (0.5 * (self.fRec59[0] * fTemp29)))))));
			...
			self.fRec3[0] = ((self.fVec12[1] * fTemp140) + (((fTemp135 / fTemp141) - (self.fRec3[1] * fTemp142)) / fTemp139));
			...
			self.fRec2[0] = (self.fRec3[0] - (((self.fRec2[2] * fTemp143) + (2.0 * (self.fRec2[1] * fTemp145))) / fTemp146));
			let mut fTemp147: F32 = (0.0 - (2.0 / fTemp144));
			self.fRec60[0] = ((self.fConst1 * (fTemp3 * self.fRec60[1])) + (fSlow42 * fTemp4));
			let mut fTemp148: F32 = F32::powf(10.0, F32::log10(F32::powf(8.0, (0.0399999991 * self.fRec60[0]))));
			self.fRec62[0] = (0.0 - (((fTemp142 * self.fRec62[1]) - (fTemp136 + self.fVec12[1])) / fTemp139));
			self.fRec61[0] = (self.fRec62[0] - (((fTemp143 * self.fRec61[2]) + (2.0 * (fTemp145 * self.fRec61[1]))) / fTemp146));
			let mut fTemp149: F32 = (((((self.fRec2[1] * fTemp147) + (self.fRec2[0] / fTemp144)) + (self.fRec2[2] / fTemp144)) * fTemp148) + (self.fRec61[2] + (self.fRec61[0] + (2.0 * self.fRec61[1]))));
			self.fRec63[0] = ((self.fConst1 * (fTemp3 * self.fRec63[1])) + (fSlow43 * fTemp4));
			let mut fTemp150: F32 = (0.0399999991 * self.fRec63[0]);
			let mut fTemp151: F32 = F32::max(fTemp150, 0.0);
			let mut fTemp152: F32 = (*input3 as F32);
			let mut fTemp153: F32 = (fTemp152 * fTemp10);
			self.fVec13[(self.IOTA & 131071) as usize] = fTemp153;
			let mut fTemp154: F32 = (fTemp106 + (0.0399999991 * ((self.fRec7[0] * fTemp30) * ((fTemp152 * self.fRec8[0]) + (fTemp10 * ((fTemp18 * ((self.fVec13[((self.IOTA - iTemp13) & 131071) as usize] * fTemp15) + (fTemp16 * self.fVec13[((self.IOTA - iTemp17) & 131071) as usize]))) + (fTemp26 * ((self.fVec13[((self.IOTA - iTemp21) & 131071) as usize] * fTemp23) + (self.fVec13[((self.IOTA - iTemp24) & 131071) as usize] * fTemp25)))))))));
			self.fVec14[0] = fTemp154;
			self.fRec68[0] = ((fTemp111 * self.fVec14[1]) - (((fTemp112 * self.fRec68[1]) - (fTemp154 / fTemp108)) / fTemp110));
			self.fRec67[0] = (self.fRec68[0] - (((fTemp113 * self.fRec67[2]) + (2.0 * (fTemp115 * self.fRec67[1]))) / fTemp116));
			self.fRec70[0] = (0.0 - (((fTemp112 * self.fRec70[1]) - (fTemp154 + self.fVec14[1])) / fTemp110));
			self.fRec69[0] = (self.fRec70[0] - (((fTemp113 * self.fRec69[2]) + (2.0 * (fTemp115 * self.fRec69[1]))) / fTemp116));
			let mut fTemp155: F32 = (2.0 * (fTemp129 * self.fRec66[1]));
			self.fRec66[0] = ((((((fTemp117 * self.fRec67[1]) + (self.fRec67[0] / fTemp114)) + (self.fRec67[2] / fTemp114)) + (fTemp118 * (self.fRec69[2] + (self.fRec69[0] + (2.0 * self.fRec69[1]))))) / fTemp116) - (((fTemp128 * self.fRec66[2]) + fTemp155) / fTemp131));
			let mut fTemp156: F32 = ((fTemp155 + (self.fRec66[0] * fTemp133)) + (fTemp134 * self.fRec66[2]));
			let mut fTemp157: F32 = (fTemp156 / fTemp131);
			self.fVec15[0] = fTemp157;
			self.fRec65[0] = ((fTemp140 * self.fVec15[1]) + (((fTemp156 / fTemp141) - (fTemp142 * self.fRec65[1])) / fTemp139));
			self.fRec64[0] = (self.fRec65[0] - (((fTemp143 * self.fRec64[2]) + (2.0 * (fTemp145 * self.fRec64[1]))) / fTemp146));
			self.fRec72[0] = (0.0 - (((fTemp142 * self.fRec72[1]) - (fTemp157 + self.fVec15[1])) / fTemp139));
			self.fRec71[0] = (self.fRec72[0] - (((fTemp143 * self.fRec71[2]) + (2.0 * (fTemp145 * self.fRec71[1]))) / fTemp146));
			let mut fTemp158: F32 = ((fTemp148 * (((fTemp147 * self.fRec64[1]) + (self.fRec64[0] / fTemp144)) + (self.fRec64[2] / fTemp144))) + (self.fRec71[2] + (self.fRec71[0] + (2.0 * self.fRec71[1]))));
			let mut fTemp159: F32 = F32::min(fTemp150, 0.0);
			let mut fTemp160: F32 = ((fTemp149 * (1.0 - fTemp151)) + (fTemp158 * (0.0 - fTemp159)));
			*output0 = ((0.0199999996 * ((self.fRec0[0] * fTemp160) / fTemp146)) as F32);
			let mut fTemp161: F32 = ((fTemp149 * fTemp151) + (fTemp158 * (fTemp159 + 1.0)));
			*output1 = ((0.0199999996 * ((self.fRec0[0] * fTemp161) / fTemp146)) as F32);
			...
			let mut fTemp162: F32 = F32::abs(F32::min(1.0, (0.0399999991 * ((*input0 as F32) * self.fRec76[0]))));
			self.fRec77[0] = ((self.fConst1 * (fTemp3 * self.fRec77[1])) + (fSlow46 * fTemp4));
			let mut fTemp163: F32 = (2.0 * insert_faustpower2_f((0.0199999996 * self.fRec77[0])));
			...
			let mut fTemp166: F32 = if (((self.fRec74[1] > fTemp162) as i32) as i32 != 0) { fTemp165 } else { 0.0 };
			self.fRec75[0] = ((self.fRec75[1] * fTemp166) + (fTemp162 * (1.0 - fTemp166)));
			self.fRec74[0] = self.fRec75[0];
			let mut fTemp167: F32 = (1.0 - self.fRec74[0]);
			*output2 = ((0.0199999996 * (((self.fRec73[0] * fTemp167) * fTemp160) / fTemp146)) as F32);
			let mut fTemp168: F32 = F32::abs(F32::min(1.0, (0.0399999991 * ((*input1 as F32) * self.fRec76[0]))));
			let mut fTemp169: F32 = if (((self.fRec78[1] > fTemp168) as i32) as i32 != 0) { fTemp165 } else { 0.0 };
			self.fRec79[0] = ((self.fRec79[1] * fTemp169) + (fTemp168 * (1.0 - fTemp169)));
			self.fRec78[0] = self.fRec79[0];
			let mut fTemp170: F32 = (1.0 - self.fRec78[0]);
			*output3 = ((0.0199999996 * (((self.fRec73[0] * fTemp170) * fTemp161) / fTemp146)) as F32);
			self.fRec80[0] = ((self.fConst1 * (fTemp3 * self.fRec80[1])) + (fSlow47 * fTemp4));
			*output4 = ((0.0199999996 * (((self.fRec80[0] * fTemp167) * fTemp160) / fTemp146)) as F32);
			*output5 = ((0.0199999996 * (((self.fRec80[0] * fTemp170) * fTemp161) / fTemp146)) as F32);
			self.fRec81[0] = ((self.fConst1 * (fTemp3 * self.fRec81[1])) + (fSlow48 * fTemp4));
			*output6 = ((0.0199999996 * (((self.fRec81[0] * fTemp167) * fTemp160) / fTemp146)) as F32);
			*output7 = ((0.0199999996 * (((self.fRec81[0] * fTemp170) * fTemp161) / fTemp146)) as F32);
