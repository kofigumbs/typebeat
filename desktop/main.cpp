...
int main(int argc, char* argv[]) {
    ...
    run(root, captureDeviceName, playbackDeviceName, [root](groovebox::Sequencer* sequencer) {
        ...
        view.bind("fromUi", [sequencer](std::string data) -> std::string {
            auto f = sequencer->handlers.find(data.substr(data.find_first_of("\"") + 1, data.find_last_of("\"") - 2));
            if (f != sequencer->handlers.end())
                sequencer->events.push({ f->second, std::stoi(data.substr(data.find_last_of(",") + 1)) });
            return "";
        });
