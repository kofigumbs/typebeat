...
export const actions = (state) => new Map([
  ...bind.oneOf('YUIOP', 'eqBand', bands, state),
  ...bind.oneOf('NM', 'eqFilter', ['freq.', 'res.'], state),
  ...bind.nudge(
    () => state[bind.join(state.eqBand, state.eqFilter)],
    i => state.send(bind.join(state.eqBand, state.eqFilter), i)
  ),
]);
